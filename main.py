...
def check(condition: bool, name: str, error_message: str):
    ...
    if condition:
        ...
        if not config.ignore_failures:
            ...
            print_slow('However, you will probably encounter a curses error if you do so.', delay=0.005)
            print('\033[0m', end='')
            sys.exit(1)
        else:
...
if config.show_animation:
    ...
    for i, row in enumerate(calc_first_frame(height, width)):
        print()
        print_slow(row, delay=max(0.01 / (i + 1), 0.0005), prefix=f'\033[38;5;{FG}m\033[48;5;{BG}m', suffix='\033[0m',
